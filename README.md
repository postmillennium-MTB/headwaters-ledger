...
- **The map's terrain is two bands, not three, and the split was found by measuring, not eyeballing.** It shipped with elevation bins at 1,000/2,000/3,000 m stacked at opacity .07/.09/.12. The top bin turned out to be dead weight: ground above 3,000 m covers 0.4% of the frame (a handful of Rockies and Elk Mountains summits) for 1,657 bytes and 143 vertices, and separates the six named regions from everything around them by only 0.8 points more than the 2,000 m break already does — measured as (% of in-frame pixels above the threshold that fall inside the six regions) minus (% that fall outside them). Dropped it, and widened the top band to catch everything above 2,000 m instead. The remaining opacity step also went from .09→.12 (a 1.3× ratio, close to imperceptible against a background wash) to .09→.20 (2.2×), so the second tier reads as *higher ground*, not a slightly darker smudge on the first.

  A **local-relief metric was tried and rejected** before settling on raw elevation — the hypothesis being that "ruggedness" (a sliding-window max-min) might track "mountainous" better than absolute height. It measured worse at every threshold tested, and at 700 m+ of local relief the separation gap went *negative*: the Cascades, the Sierra and the Coast Ranges — all outside the six regions — are rougher terrain than the plateau along the Divide corridor, so relief highlights the wrong mountains for this map's argument.

  A **750 m threshold was also tried and rejected, despite scoring best** (75.3% of pixels above it fall inside the six regions vs. 45.1% outside, a 30.2-point gap — the widest of anything tested). At 58.8% of the frame lit up, it stops reading as "high ground" and starts reading as the base map itself. The metric that best *separates* the regions is not automatically the threshold that best *communicates* "roof of the continent" — past a certain coverage, more separation makes a worse map.
...
5. **Globe terrain** — the map got a two-band elevation wash; the globe hasn't. A single global 2,000 m band would show the same "roof of the continent" fact at planetary scale, and could run higher-contrast than the map's since there's no state-outline structure competing for attention. One open question before it's worth the ~8-minute global DEM build: at 2,000 m, Antarctica's and Greenland's ice sheets light up as brightly as anywhere else on Earth, and nobody has decided yet whether to mask them out or accept it. Rivers and lakes on the globe are a smaller, independent follow-on.
